<template>
  <div style="height:100%">
    <headerTop :title="title">
     ...
     <search @search="searchCheck" style="margin-top:1rem;"></search>
    <div style="position: relative;height:100%;">
      <scroller :on-refresh="refresh" :on-infinite="infinite" infinite-scroll-disabled="busy" ref="myscroller">
        <ul class="planList">
          <li @click="planDetail(item)" v-for="( item ,index) in planList" :key="index">
            <p>{{item.sppName}}</p>
            <!-- <p>分部分项: {{item.projectName}}</p> -->
            <p>
              检查人:
              <span>{{item.sppCheckUserName}}</span>
            </p>
            <p>
              检查项:
              <!-- <span>{{item.qppContent}}</span> -->
              <ul v-for="(value,index) in (item.hdContent||'').split(',')" :key="index" style="padding-left:.2rem">{{++index+':'+value}}</ul>
            </p>
            <p>
              巡检部门:
              <!-- <span>{{item.qppDepartmentName}}</span> -->
              <ul v-for="(value,index) in (item.sppDepartmentName||'').split(',')" :key="index" style="padding-left:.2rem">{{++index+':'+value}}</ul>
            </p>
            <p>
              填报日期:
              <span>{{item.sppCreateDateTime|dateformat}}</span>
            </p>
          </li>
        </ul>
      </scroller>
...
import search from "@/components/Plansearch.vue";
import qs from "qs";
import { mapGetters } from "vuex";
import {safetywu} from "@/api/request.js"
export default {
  components: {
    headerTop,
    search
    ...
    created() {
    if (this.filterData) {
      this.formData.sppBeginDate = this.filterData.spBeginDate;
      this.formData.sppEndDate = this.filterData.spEndDate
    }
    this.getList()
  },
    data() {
  ...
  methods: {
    searchCheck(data) {
      ...
      this.getList();
        ...
        planDetail(item) {
      this.$router.push({ path: "PlanDetail", query: { id: item.id } });
    },
        // 获取数据
    getList() {
      this.$nextTick(()=>{
        this.formData.sppBeginDate="",
        this.formData.sppEndDate=""
      })
      var that = this;
